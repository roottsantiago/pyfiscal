...
class BaseGenerator(object):
	...
	def parse(self, nombres, paterno, materno=None, estado=None):
		...
		if materno is not None:
			...
			self.materno = util.quita_articulo(self.materno)
			self.materno = util.quita_CH_LL(self.materno)

		self.nombres = util.upper(nombres)
		self.paterno = util.upper(paterno)
	
		self.nombres = util.quita_nombre(self.nombres)
		self.paterno = util.quita_articulo(self.paterno)

		self.nombres = util.quita_CH_LL(self.nombres)
		self.paterno = util.quita_CH_LL(self.paterno)
	...
	def iniciales_nombre(self, nombres, paterno, materno):
		...
		vocal = util.busca_vocal(paterno)
		# Agrega vocal
		iniciales += vocal 
	...
	def consonante_curp(self, param):
		consonante = util.busca_consonante(param)
		return consonante

	def anio_fecha(self, fecha):
		anio = util.get_anio(fecha)
		return anio
